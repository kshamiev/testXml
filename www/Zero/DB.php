...
class Zero_DB
{
     ...
     * @throws Exception
     */
    public static function Escape_Format($sql, $params)
    {
        preg_match_all("~(%[d|s|v|f]{1})~si", $sql, $match);
        if ( count($params) != count($match[1]) )
        ...
        foreach ($match[1] as $k => $v)
        {
            ...
            switch ( $v )
            {
                case '%d':
                {
                    $params[$k] = self::Escape_I($params[$k]);
                    break;
                }
                case '%f':
                {
                    $params[$k] = self::Escape_F($params[$k]);
                    break;
                }
                case '%s':
                {
                    $params[$k] = self::Escape_T($params[$k]);
                    break;
                }
            }
            //            echo $v . " = " . $arr[$k] . "<br>";
        }
        return preg_replace($match[1], $params, $sql, 1);
    }
